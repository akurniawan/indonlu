...
class SimpleTokenizer:
    ...
    def encode(self, text, text_pair=None, add_special_tokens=False):
        ...
        if not add_special_tokens:
            ...
            tokens = (
                [self.vocab["<cls>"]]
                + [self.vocab[tok] for tok in self.tokenizer.tokenize(text)]
                + [self.vocab["<sep>"]]
            )
            if text_pair:
...
def load_model(args):
    if 'bert-base-multilingual' in args['model_checkpoint']:
        ...
        if 'sequence_classification' == args['task']:
            model_class = (
                AlbertForSequenceClassification if 'lite' in args['model_checkpoint'] else BertForSequenceClassification
            )
        elif 'token_classification' == args['task']:
            model_class = (
                AlbertForWordClassification if 'lite' in args['model_checkpoint'] else BertForWordClassification
            )
        elif 'multi_label_classification' == args['task']:
            model_class = (
                AlbertForMultiLabelClassification
                if 'lite' in args['model_checkpoint']
