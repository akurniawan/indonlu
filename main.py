...
from utils.args_helper import append_dataset_args, get_parser
from utils.functions import load_model

BASE_PATH = Path(os.getcwd())
LOG_PATH = Path(os.path.join(BASE_PATH, "tensorboard_logs"))
...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        print(
            "(Epoch {}) TRAIN LOSS:{:.4f} {} LR:{:.8f}".format(
                (epoch + 1), total_train_loss / (i + 1), metrics_to_string(metrics), get_lr(args, optimizer)
            )
        )
...
if __name__ == "__main__":
    ...
    # create directory
    model_dir = "{}/{}/{}/{}".format(
        args["model_dir"], args["dataset_class"].split(".")[-1], args["experiment_name"], model_checkpoint_name
    )
